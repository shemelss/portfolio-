    ...
    try {
      ...
      if (referralCode) {
        ...
        if (referrerFound) {
          ...
          // Referral codes are optional. Unknown codes are kept on the account
          // for tracking, but they never block registration or award a bonus.
          console.log("No matching referral code found; continuing registration")
        }
      }
      ...
      if (referralCode && referrerFound && referrer) {
        ...
        const updatedUsers = users.map((u: any) => {
          if (u.referralCode && u.referralCode.trim().toUpperCase() === referralCode.trim().toUpperCase()) {
            // Award the referrer once when a new account joins with their code.
            ...
            return { ...u, referrals, balance: (Number(u.balance) || 0) + 5 }
          }
          return u
        })
        ...
        {referralCode && !referrerInfo && (
          <Alert className="py-2 bg-yellow-50 dark:bg-yellow-950 border-yellow-200 dark:border-yellow-800">
            ...
            <AlertDescription className="text-xs text-yellow-800 dark:text-yellow-300">
              Referral codes are optional. If the code matches a member, the referral bonus will be applied automatically.
            </AlertDescription>
          </Alert>
        )}
